...
bool esp8266_init(bool reset) {
    ...
    // 直接連線到指定 WIFI 下，請更改下方 \"\" 中的 SSID 與 password
    char cmd[100] = "AT+CWJAP_CUR=\"IB811_2.4G\",\"chaos811\"";
    // 手動在 Console 中選擇連線 AP 時，將上面 char cmd[100] 加入註解，並取消下方 11 列註解
...
char * esp8266_listen() {
	// +IPD 標頭最大 12 Bytes + 封包內容最大 2048 Bytes
	alt_u8 buffer[2060];
	alt_u8 *packet;
	int length = 0;
	int i;
